...
function GuestSearchPredictive() {
  ...
  const fetchGuests = (pageNumber = 1) => {
    fetch(`/api/guests/search?page=${pageNumber}`)
      .then((res) => {
      ...
      .catch((err) => setError(err.message || "Error loading guests"));
  };
  ...
  useEffect(() => {
    ...
    debounceTimeout.current = setTimeout(() => {
      fetch(`/api/guests/search?q=${encodeURIComponent(query.trim())}&page=1`)
        .then((res) => {
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "600px", position: "relative" }}>
      ...
      {!query.trim() && (
        <>
          <h2 style={{ marginTop: "2rem" }}>All Guests</h2>
          <ul>
            {allGuests.map((guest) => (
...
export default GuestSearchPredictive;
